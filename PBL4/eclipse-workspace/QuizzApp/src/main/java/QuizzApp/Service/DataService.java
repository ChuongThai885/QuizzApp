...
public class DataService {
	...
	public User_Infor Get_User(String Email)
	{
		...
		try (PreparedStatement st = CreateConnect().prepareStatement(query))
		{
			...
			while(res.next())
			{
				user = new User_Infor(res.getLong("ID"),res.getString("User_Name"),res.getString("Email"),res.getString("Pass"),res.getTimestamp("SignUp_Date"));
			}
			return user;
		}
		catch (Exception e) {
	...
	public List<Answer> Get_ListAnswerByQuesID(int ID_Question)
	{
		return null;
	}
