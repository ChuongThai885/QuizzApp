...
io.on('connection', (socket) => {
    ...
    socket.on('create-room', (cb) => // when manager request server create new room
    {
        try {
    ...
    socket.on('joining-room', (id_room, cb) => // when player request to join room
    {
        try {
    ...
    socket.on('set-name', (idRoom, name_user, cb) => // when player set it's name
    {
        try {
    ...
    socket.on('start-game', (quizz) => //when manager request server start game
    {
        try {
            for (var r of IDRoom) {
                i = parseInt(r);
    ...
    socket.on('send-result', (id_Room, answer, score) => //when player select any answer on form or time out
    {
        try {
            var idRoom = parseInt(id_Room);
    ...
    socket.on('next-question', (idRoom) => //when manager request server send next queston, send to all people in room
    {
        try {
            if (RoomList[parseInt(idRoom)].IDAdmin == socket.id) {
    ...
    socket.on('change-state-room', (idRoom) => //when manager request to lock or unclock room
    {
        try {
    ...
    socket.on('disconnect', () => // if manager of room disconnected, remove room
    {
        for (var r of IDRoom) {
            console.log(r);
            i = parseInt(r);
